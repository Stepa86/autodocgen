...
Функция ВыполнитьГенерациюДокументации(ПараметрыКоманды, ТекстОшибки) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИсходныйФайл) Тогда
		...
		ТекстОшибки = "Не указан путь к файлу";
		Возврат Ложь;
	...
	Если Не Файл.Существует() ИЛИ Файл.ЭтоКаталог() Тогда
		...
		ТекстОшибки = СтрШаблон("Исходный файл '%1' не существует или это каталог", ИсходныйФайл);
		Возврат Ложь;
			
	КонецЕсли;
		
	НастройкиСтенда = ОбщегоНазначения.ПрочитатьНастройкиСтенда(Файл.Путь, ПараметрыКоманды.ФайлНастроек);
	НастройкиАнализаИзменений = ОбщегоНазначения.ПрочитатьНастройкиАнализаИзменений(НастройкиСтенда,
		ПараметрыКоманды.ИсходныйФайл, ПараметрыКоманды.РежимGit);
