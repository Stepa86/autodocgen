...
Процедура ОписаниеКоманды(КомандаПриложения) Экспорт
	...
	КомандаПриложения.Опция("push-also", ,
...
Функция ПолучитьСтруктуруПараметров(Знач КомандаПриложения)
	...
	ПараметрыКоманды.Вставить("КаталогДополнительнойДокументации", КомандаПриложения.ЗначениеОпции("push-also"));
...
Функция ВыполнитьГенерациюДокументации(ПараметрыКоманды, ТекстОшибки)
	...
	Если ПараметрыКоманды.ГенерацияПриНаличииОшибок ИЛИ НЕ ЕстьОшибкиГенерации Тогда
		...
		Если НЕ ПустаяСтрока(ПараметрыКоманды.КаталогДополнительнойДокументации) И ПустаяСтрока(ОшибкиПубликации) Тогда
			
			Если ПараметрыКоманды.Формат = "confluence" Тогда
				
				ГенераторСодержимого.ОпубликоватьФайлыКаталога(
					ПараметрыКоманды.КаталогДополнительнойДокументации, 
					РезультатПубликации.ОшибкиПубликации);
				
			Иначе
				
				Лог = МенеджерПриложения.ПолучитьЛог();
				Лог.Предупреждение("Публикация ручной документации поддерживается только для Confluence");
