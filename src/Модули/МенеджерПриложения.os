...
Процедура ЗарегистрироватьКомандыПриложения()
	...
	Для Каждого ФайлКоманды Из ФайлыКоманд Цикл
		...
		ИмяКлассаРеализации = ФайлКоманды.ИмяБезРасширения;

		Если СтрСравнить(ИмяКлассаРеализации, "КомандаСправкаПоПриложению") = 0 Тогда
			Продолжить;
		КонецЕсли;

		РеализацияКоманды = Новый(ИмяКлассаРеализации);
		ИмяКоманды = РеализацияКоманды.ИмяКоманды();

		Приложение.ДобавитьКоманду(ИмяКоманды + " " + Лев(ИмяКоманды, 1),
									РеализацияКоманды.КраткоеОписаниеКоманды(),
									РеализацияКоманды);
