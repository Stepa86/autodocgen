		...
		Если НЕ НастройкиГенератора.НастройкиАнализаИзменений.Анализировать 
			...
			Иначе
				...
				Если Раздел = Неопределено Тогда
					...
					Раздел.Содержимое = СтрШаблон(Шаблоны.ШаблонСтраницыРаздела, ОбработатьСтроку(СтруктураПодсистем.ОписаниеРаздела));
				...
				Если Подсистема = Неопределено Тогда
					...
					Подсистема.Содержимое = СтрШаблон(Шаблоны.ШаблонСтраницыПодсистемы, ОбработатьСтроку(СтруктураПодсистем.ОписаниеПодсистемы));
...
Функция Опубликовать(РезультатГенерации, НастройкиГенератора) Экспорт
	...
	Если РезультатГенерации.СоздаваемыеРазделы.Количество()
		И РезультатГенерации.СоздаваемыеСтраницы.Количество() Тогда
		
		ИдентификаторКорняСтраницы = Confluence.НайтиСтраницуПоИмени(ПодключениеConfluence, ПространствоConflunece, КорневаяСтраницаConflunece);
		Если ПустаяСтрока(ИдентификаторКорняСтраницы) Тогда
			
			ОшибкиПубликации = ОшибкиПубликации + ?(ПустаяСтрока(ОшибкиПубликации), "", Символы.ПС) 
								+ "В пространстве отсутствует корневая страница документации '" + КорневаяСтраницаConflunece + "'";
			
		Иначе
			
			Для Каждого РазделПодсистема Из РезультатГенерации.СоздаваемыеРазделы Цикл
				
				ИдентификаторСтраницы = Confluence.НайтиСтраницуПоИмени(ПодключениеConfluence, ПространствоConflunece, РазделПодсистема.ИмяСтраницы);
				
				Если ПустаяСтрока(ИдентификаторСтраницы) Тогда
					
					Если РазделПодсистема.Родитель = Неопределено
						ИЛИ ЗначениеЗаполнено(РазделПодсистема.Родитель.Идентификатор) Тогда
						
						ТекстОшибкиСозданияСтраницы = "";
						Попытка 
							
							ИдентификаторСозданнойСтраницы = Confluence.СоздатьСтраницу(
																ПодключениеConfluence,
																ПространствоConflunece,
																РазделПодсистема.ИмяСтраницы,
																РазделПодсистема.Содержимое,
																?(РазделПодсистема.Родитель = Неопределено, 
																	ИдентификаторКорняСтраницы, 
																	РазделПодсистема.Родитель.Идентификатор));
							
						Исключение
							
							ОшибкиПубликации = ОшибкиПубликации + ?(ПустаяСтрока(ОшибкиПубликации), "", Символы.ПС) 
												+ "Ошибка создания страницы '" + РазделПодсистема.ИмяСтраницы + "': " + ОписаниеОшибки();
							
							Продолжить;
							
						КонецПопытки;
						
						РазделПодсистема.Идентификатор = ИдентификаторСозданнойСтраницы;
						
					Иначе
						
						ОшибкиПубликации = ОшибкиПубликации + ?(ПустаяСтрока(ОшибкиПубликации), "", Символы.ПС) 
											+ "Создание страницы подсистемы '" + РазделПодсистема.ИмяСтраницы + "' невозможно, т.к. не создана страница раздела";
				...
				Иначе
					...
					РазделПодсистема.Идентификатор = ИдентификаторСтраницы;
	...
	Для Каждого НоваяСтраница Из РезультатГенерации.СоздаваемыеСтраницы Цикл
		...
		Если НЕ ЗначениеЗаполнено(НоваяСтраница.Родитель.Идентификатор) Тогда
			
			ОшибкиПубликации = ОшибкиПубликации + ?(ПустаяСтрока(ОшибкиПубликации), "", Символы.ПС) 
								+  "Создание страницы '" + НоваяСтраница.ИмяСтраницы + "' невозможно, т.к. не создана страница подсистемы";
		...
		Попытка
			...
			Confluence.СоздатьСтраницуИлиОбновить(
								ПодключениеConfluence,
								ПространствоConflunece, 
								НоваяСтраница.ИмяСтраницы,
								СокрЛП(НоваяСтраница.Содержимое),
								НоваяСтраница.Родитель.Идентификатор, 
	...
	Если НЕ ПустаяСтрока(РезультатГенерации.СодержимоеСтраницыКонстант) Тогда
		...
		Попытка
			...
			Confluence.СоздатьСтраницуИлиОбновить(
					...
					ПространствоConflunece, 
					"Константы",
					СокрЛП(РезультатГенерации.СодержимоеСтраницыКонстант),
					ИдентификаторКорняСтраницы, 
					ОбновлятьИзмененныеСтраницы);
...
Функция ПрочитатьНастройки(НастройкиСтенда) Экспорт
	...
	Если ЗначениеЗаполнено(НастройкиConfluence) Тогда
		...
		Попытка
			...
			Шаблоны = ЗагрузитьШаблоны(НастройкиConfluence["ПутьКШаблонам"]);
...
Функция СформироватьТекстСтраницыПоОписанию(ОписаниеСтруктурыМодуля, ТекстОшибок, Шаблоны = Неопределено)
	...
	Если ОписаниеСтруктурыМодуля.Родитель.Тип = "Constant" Тогда
	...
	Для Каждого Блок Из ОписаниеСтруктурыМодуля.НаборБлоков Цикл
		...
		Если Блок.ТипБлока = ТипБлоковМодуля.ЗаголовокПроцедуры
			...
			Если Блок.ОписаниеБлока.ИмяРаздела = ТипОбласти.РазделПрограммныйИнтерфейс Тогда
				...
				+ СтрШаблон(Шаблоны.ШаблонЗаголовок, Блок.ОписаниеБлока.ИмяМетода);
...
Функция ПолучитьСтруктуруПодсистем(Подсистемы)
	...
	Для Каждого Подсистема Из Подсистемы Цикл
		...
		Если НЕ ПустаяСтрока(АнализироватьТолькоПотомковПодсистемы) И Не СтрНачинаетсяС(Подсистема.ПодсистемаИмя, "ПодсистемыБСП") Тогда
...
Функция ЗагрузитьШаблоны(ПутьКШаблонам, ПустойШаблон = ЛОЖЬ)
	...
	Если НЕ ЗначениеЗаполнено(ПутьКШаблонам) Тогда
		...
		ПутьКШаблонам = ОбъединитьПути(СтартовыйСценарий().Каталог, "additional", "Шаблоны_conluence.json");
