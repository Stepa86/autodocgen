...
Функция СгенерироватьПоФайлу(НастройкиГенератора) Экспорт
	...
	ИначеЕсли ДанныеМодуля <> Неопределено Тогда
		...
		Если НЕ ПустаяСтрока(Содержимое) Тогда
			...
			СтрокаОписания.Имя = ЧтениеОписанийБазовый.ПолноеИмяОбъекта(Модуль, Истина);
...
Функция ПодготовитьДанныеПубликации(НастройкиГенератора, ТолькоСтруктура)
	...
	Для Каждого Модуль Из НастройкиГенератора.Парсер.ОписаниеКонфигурации().Модули Цикл
		...
		Иначе
			...
			Если НЕ ОбработатьМодуль(Модуль, НастройкиГенератора, Результат)
...
Функция ПроверитьПодсистемыМодуля(Модуль, Ошибки)
	...
	Если ПустаяСтрока(СтруктураПодсистем.ИмяРаздела) Тогда
		...
		Ошибки.Добавить(Модуль.ПутьКФайлу + ": ошибочная структура подсистем или модуль пропущен по фильтру подсистем");
		Возврат Ложь;
		
	КонецЕсли;

	Если СтруктураПодсистем.ИмяРаздела= "SKIP" Тогда
		
		// Сработал фильтр по подсистеме. Модуль не проходит, но ошибкой не является
		Возврат Ложь;
...
Функция ПолучитьСтруктуруПодсистем(Подсистемы)
	...
	СтруктураПодсистем.ИмяРаздела = "SKIP"; // Если подсистемы существуют, но в настройках стоит фильтр.
