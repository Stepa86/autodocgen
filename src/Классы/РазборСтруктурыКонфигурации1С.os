...
Функция ПолучитьИмяТипаНаРусском(ИмяТипа) Экспорт
	...
	ИначеЕсли ИмяТипа = "AccumulationRegister" Тогда

		ИмяТипаРус = "РегистрНакопления";

	ИначеЕсли ИмяТипа = "CalculationRegister" Тогда

		ИмяТипаРус = "РегистрРасчета";
	
	ИначеЕсли ИмяТипа = "ChartOfCharacteristicTypes" Тогда

		ИмяТипаРус = "ПланВидовХарактеристик";

	ИначеЕсли ИмяТипа = "ChartOfCalculationTypes" Тогда

		ИмяТипаРус = "ПланВидовРасчета";
...
Процедура ЗаполнитьИменаФайлов(ОбъектыКонфигурации, КаталогИсходников, ИерархическийФормат)
	...
	Для Каждого СтрокаОбъектКонфигурации Из ОбъектыКонфигурации Цикл
		...
		ИначеЕсли ИерархическийФормат И СтрокаОбъектКонфигурации.Тип = "ChartOfCalculationTypes" Тогда
			
			ИмяТипа = "ChartsOfCalculationTypes";
		...
		Если НЕ СтрокаОбъектКонфигурации.Тип = "Configuration" Тогда
			...
			Если НЕ ФайлФайлСОписанием.Существует() Тогда
				...
				Если Переименования = Неопределено Тогда
					...
					Текст = Новый ТекстовыйДокумент;
					Текст.Прочитать(ФайлПереименований, КодировкаТекста.UTF8NoBOM);
	...
	Для НомерСтроки = 1 По КоличествоСтрокМодуля Цикл
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ТекущийБлок) Тогда
				...
				УдалятьКомментарии = ТекущийБлок = ТипБлоковМодуля.ЗаголовокПроцедуры ИЛИ ТекущийБлок = ТипБлоковМодуля.ЗаголовокФункции;
					
				Для ИтБлок = НачальнаяСтрока По КонечнаяСтрока Цикл
					
					Если УдалятьКомментарии Тогда
						
						СтрокиСодержимого.Добавить(УдалитьКомментарийИзСтроки(Файл.ПолучитьСтроку(ИтБлок)));
						
					Иначе
						
						СтрокиСодержимого.Добавить(Файл.ПолучитьСтроку(ИтБлок));
						
					КонецЕсли;
					
				КонецЦикла;
				
				НоваяЗаписьОБлоке.Содержимое = СтрСоединить(СтрокиСодержимого, Символы.ПС);
